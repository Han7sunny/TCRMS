...
public class UserService {
    ...
    public LoginResponseDto login(LoginRequestDto loginRequestDto) {
        ...
        if(findUser.isEmpty()){
//            return ResponseDto.builder().isSuccess(false).message("회원정보를 찾을 수 없습니다.").build();
            return LoginResponseDto.builder()
                    .isSuccess(false).message("회원정보를 찾을 수 없습니다.")
                    .build();
        }

        User user = findUser.get();
        return LoginResponseDto.builder()
                .userId(user.getUserId()).isFirstLogin(user.getIsFirstLogin()).token(jwtTokenProvider.createToken(user.getUserId(), user.getAuth().name())).auth(user.getAuth().name())
                .isSuccess(true)
                .build();
    }
}
